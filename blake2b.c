...
#if !defined(USE_SSE) || !defined(HAVE_SSE2)
...
static void
blake2b_set_lastnode(blake2b_ctx *ctx) {
  ctx->f[1] = (uint64_t)-1;
}

static int
blake2b_is_lastblock(const blake2b_ctx *ctx) {
  return ctx->f[0] != 0;
}

static void
blake2b_set_lastblock(blake2b_ctx *ctx) {
  if (ctx->last_node)
    blake2b_set_lastnode(ctx);

  ctx->f[0] = (uint64_t)-1;
}

static void
blake2b_increment_counter(blake2b_ctx *ctx, const uint64_t inc) {
  ctx->t[0] += inc;
  ctx->t[1] += (ctx->t[0] < inc);
}

static void
blake2b_init0(blake2b_ctx *ctx) {
  size_t i;
...
int
blake2b_init_param(blake2b_ctx *ctx, const blake2b_param *P) {
...
int
blake2b_init(blake2b_ctx *ctx, size_t outlen) {
  blake2b_param P[1];
...
int
blake2b_update(blake2b_ctx *ctx, const void *pin, size_t inlen) {
...
blake2b(
  ...
  blake2b_update(&ctx, (const uint8_t *)in, inlen);
